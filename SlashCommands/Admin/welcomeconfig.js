...
module.exports = {
  ...
  options: [
    ...
    },
    {
      name: "show",
      type: 1,
      description: "Show the informations and test the welcome message and image.",
    },
  ],
  cooldown:10,
  ...
  run: async(client,interaction,args,guildData) => {
    ...
    if (subCommand === "avatar") {
      if(!x && !y && !width && !height) return interaction.reply({content:`**🙄 - You must type anything at least.**`})
      ...
      if(!x && !y) return interaction.reply({content:`**🙄 - You must type anything at least.**`})
      ...
      guildData.save()
      return interaction.reply({ content: `**✅ - message toggle has been ${toggle ? "Disabled" : "Enabled"}.**` })
    }
    else if (subCommand === "toggle_image") {
      let toggle = guildData.welcome.toggle.image;
      guildData.welcome.toggle.image = toggle ? false : true
      guildData.save()
      return interaction.reply({ content: `image toggle has been ${toggle ? "Disabled" : "Enabled"}` })
    } 
    else if (subCommand === "channel") {
      let channel = interaction.options.getChannel("channel")
guildData.welcome.channel = channel.id
      guildData.save()
      interaction.reply({ content: `**✅ - #${channel.name} has been set for welcome channel.**`})
    }
...
> [invites] => the inviter invites number.**`)
      ...
      let collector = await interaction.channel.awaitMessages({
        ...
        if(message.content.length > 250) return interaction.editReply(`**🙄 - The message must be less than 250 characters.**`)
      ...
      if(!x && !y && !text) return interaction.reply({content:`**🙄 - You must type anything at least.**`})
      
      if(text?.split(" ").length > 5) return interaction.reply(`the text must be less than 5 words`)
      ...
      guildData.welcome.background = pics
      guildData.save()
      interaction.reply({content:`**✅ - The welcome background has been set.**`,files:[pic]})
